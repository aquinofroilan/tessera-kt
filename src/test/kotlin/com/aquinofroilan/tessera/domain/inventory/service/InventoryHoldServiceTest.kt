...
class InventoryHoldServiceTest {
    ...
    fun `should place hold on non-serialized product`() {
        val product = Product(id = productId, organizationId = orgId, sku = "P1", name = "Prod", listPrice = BigDecimal.TEN, priceCurrency = "USD")
    ...
    fun `should place hold on serialized product`() {
        val product = Product(id = productId, organizationId = orgId, sku = "P1", name = "Prod", listPrice = BigDecimal.TEN, priceCurrency = "USD", isSerialized = true)
        val serials = listOf("SN1", "SN2")
        `when`(productRepository.findById(productId)).thenReturn(Optional.of(product))
        val s1 = ProductSerial(organizationId = orgId, productId = productId, serialNumber = "SN1", status = SerialStatus.IN_STOCK, currentWarehouseId = warehouseId)
        val s2 = ProductSerial(organizationId = orgId, productId = productId, serialNumber = "SN2", status = SerialStatus.IN_STOCK, currentWarehouseId = warehouseId)
        
        `when`(productSerialRepository.findByOrganizationIdAndProductIdAndSerialNumberIn(orgId, productId, serials)).thenReturn(listOf(s1, s2))
