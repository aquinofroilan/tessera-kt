...
def fix_errors(log_path):
    ...
    for file_path, errors in files_to_fix.items():
        ...
        for line_num, msg in errors:
            ...
            if "actual type is 'UUID!', but 'String" in msg or "expected 'String', actual 'UUID!'" in msg or "actual type is 'UUID!', but 'String?' was expected." in msg:
                # We placed a UUID where a String was expected. Revert it to a string.
                line = re.sub(r'java\.util\.UUID\.fromString\("([^"]+)"\)', r'"\1"', line)
                
            elif "actual type is 'String', but 'UUID" in msg or "expected 'UUID', actual 'String'" in msg or "actual type is 'String', but 'UUID?' was expected" in msg or "expected 'UUID?', actual 'String'" in msg:
